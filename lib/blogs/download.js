...
module.exports = function *(argv) {
  ...
  yield asyncEach(blogs, function *(blog, idx) {
    ...
    yield asyncEach(articles, function *(article, jdx) {
      let jsonKey = `blogs/${blog.handle}/${article.id}-${article.title}/article.json`
      let htmlKey = `blogs/${blog.handle}/${article.id}-${article.title}/article.html`
      ...
      yield fs.writeFileAsync(`blogs/${blog.handle}/${article.id}-${article.title}/metafields.json`, JSON.stringify(metafields))
    })
